...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        l1_output = X @ W1 + b1
        l1_relu = np.where(l1_output < 0, 0, l1_output)
        scores = l1_relu @ W2 + b2
        ...
        for score in scores:
            softmax_score = []
            for c in score:
                softmax_score.append(np.exp(c) / np.sum(np.exp(score)))
        ...
        # CE loss
        loss = 0
        for i, softmax_score in enumerate(softmax_scores):
        ...
        grads["W2"] = np.dot(l1_relu.T, d_scores) + reg * W2
        grads["b2"] = np.sum(d_scores, axis=0)

        d_hidden = np.dot(d_scores, W2.T)
        d_hidden[l1_relu <= 0] = 0
    ...
    def predict(self, X):
        ...
        ###########################################################################
        y_pred = np.argmax(
            self.relu(X @ self.params["W1"] + self.params["b1"]) @ self.params["W2"]
            + self.params["b2"],
            axis=1,
        )
